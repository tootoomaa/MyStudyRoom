...
protocol ChainType {
    var next: ChainType? { get set }
    func excute(event: String) -> String?
}

extension ChainType {
    mutating func setNext(obj: ChainType) {
        self.next = obj
    }
}

class MyOrder: ChainType {
    var next: ChainType?
    
    func excute(event: String) -> String? {
        if event == "Order" { return "주문을 처리합니다" }
        return self.next?.excute(event: event)
    }
}

class MyCancel: ChainType {
    var next: ChainType?
    
    func excute(event: String) -> String? {
        if event == "Cancel" { return "주문을 취소합니다." }
        return self.next?.excute(event: event)
    }
}
...
class MyHanlder {
    func event(event: String) -> String? {
        var first = MyOrder()
        first.setNext(obj: MyCancel())
        return first.excute(event: event)
    }
}

let myHandler = MyHanlder()
print(myHandler.event(event: "Cancel") ?? "")
